...
export default function DistributorMedicineInfo(props) {
  ...
  function getSupplyChainStepContent(stepIndex) {
    switch (stepIndex) {
      ...
      case 4:
        return "Les médicament sont livrés au pharmacie";
  ...
  if (loading) {
    ...
    return (
      <div>
        <h1 style={{ textAlign: "center" }}>Détails du produit</h1>
        <p>{details}</p>
        ...
        >
          Enregistrer les informations sur les médicaments dans la base de
          données
        </Button>
      </div>
    );
