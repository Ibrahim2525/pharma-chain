...
export default function ReceivedRawMaterial(props) {
  ...
  async function getRawMaterialData() {
    ...
    let display = (
      <div>
        ...
        <p>Fournisseur de produit: {data[3]}</p>
  ...
  function getSupplyChainStepContent(stepIndex) {
    switch (stepIndex) {
      ...
      case 1:
        return "La matière première collectée par le transporteur est en route vers le fabricant";
      case 2:
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        >
          Enregistrer les informations sur les matières premières dans la base
          de données
        </Button>
      </div>
    );
